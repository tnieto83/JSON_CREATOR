...
def json_creator(input):
    with open('REFS_operands_en-us.txt', encoding='latin-1') as operands:
        lst_operands = []
        ...
        with open('REFS_transcriptions_en-us.txt', encoding='latin-1') as transcriptions:
            lst_transcriptions = []
...
def grammar_tk_launcher(input):       
        ...
        file = input.rsplit('.', 1)[0]+ ".json"
        call(shlex.split('/opt/verbio/bin/grammar-tk' + ' -a ' + '//builtin_' + input + '_en-us-asr3.bnf' + ' -r ' + file + ' -o  ' + input.rsplit('.', 1)[0] + '_results.json'))
